...
def medianSmooth(time, nparray, nsmooth):#get odd eg (3, 5 smth factor)
    out = []
    n = int((nsmooth + 1)/ 2)
    t = []
    for i, val in enumerate(nparray):
...
def medianSmthIt(time, nparray, nsmooth, iterations):
    smtht = time
    smth = nparray
    for i in range(0, iterations):
        smtht, smth = medianSmooth(smtht, smth, nsmooth)
    return smtht, smth